function dataCubeDashboard() {
    viewName = 'data-cube';
    if(dashboardManager.dataCube.grid){
        dashboardManager.dataCube.grid.destroy();
    }

    const columnDefs = dataManager.data[viewName].columnDefs;
    const rowData = dataManager.data[viewName].rowData;

    dataManager.gridOptions = gridManager.getGridOptions(columnDefs, rowData);
    container = $('#data-cube');
    dataManager.grid = agGrid.createGrid(document.getElementById('data-cube'), dataManager.gridOptions);
    let s = `
